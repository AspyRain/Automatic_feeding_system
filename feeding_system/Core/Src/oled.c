...
void OLED_Showtime(void *pmt)
{
    ...
    while (1)
    {
        uint8_t DATA[14] = ""; // 修改为uint8_t数组，确保足够
        ...
        for (int i = 0; i < NUM_FLAGS; ++i)
        {
            ...
            case 0: // 年
                if (is_Change[i])
                {
                    sprintf(DATA, "%04d", timerData->date->year);
            ...
            case 1: // 月
                if (is_Change[i])
                {
                    sprintf(DATA, "%02d", timerData->date->month);
                    OLED_ShowString(48, 2, DATA, 16);
            ...
            case 2: // 日
                if (is_Change[i])
                {
                    sprintf(DATA, "%02d", timerData->date->day);
                    OLED_ShowString(80, 2, DATA, 16);
                    is_Change[i] = 0;
                }
                break;
            case 3: // 时
                if (is_Change[i])
                {
                    sprintf(DATA, "%02d", timerData->time->h);
            ...
            case 4: // 分
                if (is_Change[i])
                {
                    sprintf(DATA, "%02d", timerData->time->m);
                    OLED_ShowString(32, 4, DATA, 16);
            ...
            case 5: // 秒
                if (is_Change[i])
                {
                    sprintf(DATA, "%02d", timerData->time->s);
                    OLED_ShowString(64, 4, DATA, 16);
                    is_Change[i] = 0;
                }
                break;
            }
